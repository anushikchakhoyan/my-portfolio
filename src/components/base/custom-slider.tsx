    ...
    return (
        <>
            <div className={cn(`mx-auto flex items-center justify-between w-full max-w-3xl p-8
                           bg-zinc-50 dark:bg-secondary/30 rounded-lg shadow-sm`, className)}>
...
const SliderArrow: React.FC<SliderArrowProps> = (({ handleClick, children }) => {
    return (
        <Button
            ...
            className="text-xl text-zinc-700 hover:text-primary-600 dark:text-primary-400 dark:hover:text-primary"
        >
            {children}
        </Button>
    )
            ...
            {items.map((_: any, index: number) => (
                <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors
                             ${index === currentIndex
                            ? "bg-primary"
                            : "bg-gray-300 hover:bg-primary-300 dark:bg-zinc-700 dark:hover:bg-primary"
                        }`}
